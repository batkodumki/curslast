...
class BalanceScaleVisualizer:
    ...
    def draw_cube(self, draw, center_x, bottom_y, size, color):
        """
    ...
    def create_visual(self, impact_name, impact_value):
        ...
        # Cube root для візуального масштабування (як в оригінальному коді)
        scale_factor = (impact_value / 5.0) ** (1/3)

        # Базовий розмір кубика
        base_cube_size = 50

        # Розміри кубиків
        left_cube_size = base_cube_size  # Лівий завжди базовий
        right_cube_size = int(base_cube_size * scale_factor)

        # Нахил терезів (більше значення = більший нахил)
        if impact_value > 5.0:
            tilt_angle = -min((impact_value - 5.0) / 4.5, 1.0)  # Нахил вправо
        elif impact_value < 5.0:
            tilt_angle = min((5.0 - impact_value) / 3.5, 1.0)   # Нахил вліво
        else:
        ...
        # Намалювати кубики НА чашах
        # Кубики розміщуються на платформах чаш
        self.draw_cube(draw, left_x, left_plate_y, left_cube_size, self.primary_color)
        self.draw_cube(draw, right_x, right_plate_y, right_cube_size, self.accent_color)
